...
export default function LiquidEther({
  ...
  resolution = 0.5,
  isBounce = false,
  colors = ['#FF5A00', '#E65100', '#5A1A00'], // Default to current portfolio orange colors
  style = {},
  className = '',
  autoDemo = true,
  autoSpeed = 0.5,
    ...
    class CommonClass {
      ...
      init(container: HTMLDivElement) {
        ...
        try {
          ...
          console.warn('WebGL not supported, skipping LiquidEther', e);
          this.renderer = null;
        }
...
`;
    ...
    class Advection extends ShaderPass {
      ...
      updateWithOptions({ dt, isBounce, BFECC }: { dt: number; isBounce: boolean; BFECC: boolean }) {
    ...
    class ExternalForce extends ShaderPass {
      ...
      updateWithProps(props: { mouse_force: number; cursor_size: number; cellScale: THREE.Vector2 }) {
    ...
    class Viscous extends ShaderPass {
      ...
      updateWithOptions({ viscous, iterations, dt }: { viscous: number; iterations: number; dt: number }) {
    ...
    class Divergence extends ShaderPass {
      ...
      updateWithVel({ vel }: { vel: THREE.WebGLRenderTarget }) {
    ...
    class Poisson extends ShaderPass {
      ...
      updateWithOptions({ iterations }: { iterations: number }) {
        if (!this.uniforms) return this.props.output1;
        let p_in, p_out;
        for (let i = 0; i < iterations; i++) {
          if (i % 2 === 0) {
    ...
    class Pressure extends ShaderPass {
      ...
      updateWithProps({ vel, pressure }: { vel: THREE.WebGLRenderTarget; pressure: THREE.WebGLRenderTarget }) {
        if (!this.uniforms) return;
        this.uniforms.velocity.value = vel.texture;
    ...
    class Simulation {
      ...
      update() {
        ...
        this.advection.updateWithOptions({
          dt: this.options.dt,
          isBounce: this.options.isBounce,
          BFECC: this.options.BFECC
        });
        this.externalForce.updateWithProps({
          cursor_size: this.options.cursor_size,
        ...
        if (this.options.isViscous) {
          const result = this.viscousPass.updateWithOptions({
            viscous: this.options.viscous,
        ...
        this.divergence.updateWithVel({ vel });
        const pressure = this.poisson.updateWithOptions({
          iterations: this.options.iterations_poisson
        });
        if (pressure) {
          this.pressurePass.updateWithProps({ vel, pressure });
        }
      }
    }
    ...
    const applyOptionsFromProps = () => {
      if (!webglRef.current || !webglRef.current.output) return;
